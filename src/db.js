...
export class Path {
	...
	constructor(path: Path | string | null = '', args: ?Args) {
		...
		const updateData = snap => {
			...
			if(typeof value === 'object') for(let filter in this._filterBy) if(
	...
	child = (childPath: string) => {
		return new Path(
			this, {path: this._path + childPath}
		);
	};
	...
	addFilter = (filterBy = {}) => this.filter(
		filterBy, false
	);
...
firebase.auth().
	onAuthStateChanged(currentUser => {
		...
		_.forEach(
			user.listeners, value => value.callback()
		);
	});

export const user = {
	create: (...args: [string]) => firebase.auth().
		createUserWithEmailAndPassword(...args),
	signIn: (...args: [string]) => firebase.auth().
	...
	subscribe: (
		obj: mixed, meth?: string = 'onUpdate'
	) => {
	...
	unsubscribe: obj => {
		_.remove(
			user.listeners, listener => listener.id === obj
		);
	},
	get exists() {
